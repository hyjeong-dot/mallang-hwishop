...
export default function ProductDetailImage({ productId }: { productId: number }) {
    ...
    const { images, isLoading: isImagesLoading, error } = useProductImages(productId);
    ...
    const fallbackAlt = product?.korName || 'Product Image';

    if (isProductLoading || isImagesLoading) return <div className={styles.imageSection}>이미지를 불러오는 중...</div>;
